...
UINT MX_USBX_Device_Init(VOID *memory_ptr)
{
  ...
  /* USER CODE BEGIN MX_USBX_Device_Init1 */
	...
	if(TX_SUCCESS != tx_byte_allocate(byte_pool, (VOID **) &pointer, 1024, TX_NO_WAIT))
		return TX_POOL_ERROR;

	// Create the usbx cdc acm read thread
	if(TX_SUCCESS != tx_thread_create(&ux_cdc_read_thread, "viy5_cdc_acm_thread_entry", usbx_cdc_acm_read_thread_entry, 1, pointer, 1024, 20, 20, TX_NO_TIME_SLICE, TX_AUTO_START))
		return TX_THREAD_ERROR;
	...
	if(TX_SUCCESS != tx_thread_create(&ux_cdc_write_thread, "cdc_acm_write_usbx_app_thread_entry", usbx_cdc_acm_write_thread_entry, 1, pointer, 1024, 20, 20, TX_NO_TIME_SLICE, TX_AUTO_START))
		return TX_THREAD_ERROR;

	// Create the event flags group
	if(TX_SUCCESS != tx_event_flags_create(&EventFlag, "Event Flag"))
